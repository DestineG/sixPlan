  ...
  return <Modal open={Boolean(mode)} onOpenChange={(open) => { if (!open) void close(); }}
    ...
    <div className="ai-workflow">
      ...
      {preview && <section className="ai-step preview-step"><div className="ai-step-title"><span><Check size={14} /></span><strong>确认预览</strong></div>
        ...
        {preview.addStepCount + preview.updateStepCount + preview.removeStepCount + preview.reorderStepCount > 0 && <div className="notice">子阶段变更：新增 {preview.addStepCount}、修改 {preview.updateStepCount}、删除 {preview.removeStepCount}、重排 {preview.reorderStepCount}。</div>}
        ...
        <button className={preview.removeNodeCount + preview.removeEdgeCount + preview.removeStepCount > 0 ? 'danger-button' : 'primary-button'} disabled={busy} onClick={() => apply(preview.revisionChanged ? preview.currentRevision : undefined)}>{busy ? '正在应用' : preview.kind === 'snapshot' ? '创建新计划' : '应用增量变更'}</button>
      </section>}
    </div>
  </Modal>;
}
