...
import { buildBundlePrompt, buildRepairPrompt, buildSnapshotPrompt, buildTreeChangeSetPrompt,
  type PromptContext, type TreePromptContext } from '../ai-prompts';
...
export function AiPlanModal({ mode, areas, onClose, onApplied }: {
  ...
  const [newPlanKind, setNewPlanKind] = useState<'single' | 'tree'>('single');
  const [treeScope, setTreeScope] = useState<'current' | 'descendants' | 'custom'>('current');
  ...
  const [generatedTreeTargets, setGeneratedTreeTargets] = useState<Array<{ planKey: string; targetKeys: string[] }>>([]);
  ...
  const [preserveBundleAreas, setPreserveBundleAreas] = useState(true);
  ...
  useEffect(() => {
    ...
    setSelectedKeys([]); setGeneratedTreeTargets([]); setIncludeMarkdown(false); setAdvancedOpen(false);
    setNewPlanKind('single'); setTreeScope('current'); setPreserveBundleAreas(true);
  ...
  function changeSelection(keys: string[]) {
    ...
    setTreeScope('custom');
    setSelectedKeys(keys); setGeneratedTreeTargets([]); setPrompt(''); setRaw(''); setRepairPrompt(''); setPreview(null);
  }

  function changeMarkdownContext(checked: boolean) {
    discardPreviewSession();
    setIncludeMarkdown(checked); setGeneratedTreeTargets([]); setPrompt(''); setRaw(''); setRepairPrompt(''); setPreview(null);
  }

  function changePlan(nextPlanId: string) {
    discardPreviewSession(); setPlanId(nextPlanId); setOverview(null); setSelectedKeys([]); setGeneratedTreeTargets([]);
    setPrompt(''); setRaw(''); setRepairPrompt(''); setPreview(null);
  }

  function changeIdea(nextIdea: string) {
    if (nextIdea !== idea && (prompt || preview)) {
      discardPreviewSession(); setGeneratedTreeTargets([]); setPrompt(''); setRaw(''); setRepairPrompt(''); setPreview(null);
    }
    setIdea(nextIdea);
  }
  ...
  async function makePrompt() {
    ...
    if (mode === 'snapshot') setPrompt(newPlanKind === 'tree' ? buildBundlePrompt(idea, areaHint) : buildSnapshotPrompt(idea, areaHint));
    else {
      if (!overview) return toast.error('正在加载计划上下文');
      if (treeScope === 'custom' && !selectedKeys.length) return toast.error('请至少选择一个操作节点');
      setContextBusy(true);
      try {
        const { context } = await api<{ context: TreePromptContext }>(`/api/plans/${planId}/prompt-tree-context`, { method: 'POST',
          body: JSON.stringify({ scope: treeScope, includeMarkdown,
            ...(treeScope === 'custom' ? { targets: [{ planId, targetKeys: selectedKeys }] } : {}) }) });
        setPrompt(buildTreeChangeSetPrompt(idea, context));
        setGeneratedTreeTargets(context.plans.map((item) => ({ planKey: item.plan.key, targetKeys: item.targetKeys })));
  ...
  async function validate(content: string, sourceName = 'pasted.json') {
    ...
    catch (error) { if (error instanceof Error && error.message === 'cancelled') return; setRepairPrompt(buildRepairPrompt(content, '不是有效的 JSON 文本', mode === 'snapshot' && newPlanKind === 'tree' ? 'bundle' : mode === 'changeset' ? 'tree' : 'single')); return toast.error('不是有效的 JSON 文本'); }
    ...
    try {
      const result = await api<{ preview: ImportPreviewDto }>('/api/import-sessions/json', { method: 'POST', body: JSON.stringify({ content: json, sourceName,
        ...(mode === 'changeset' ? { targetPlanId: planId, promptTreeTargets: generatedTreeTargets } : {}) }) });
      ...
      const message = error instanceof ApiClientError ? error.message : '校验失败'; setRepairPrompt(buildRepairPrompt(json, message, mode === 'snapshot' && newPlanKind === 'tree' ? 'bundle' : mode === 'changeset' ? 'tree' : 'single')); toast.error(message);
  ...
  async function upload(file: File | undefined) {
    ...
    try {
      ...
      let query = '';
      if (mode === 'changeset') {
        form.append('promptTreeTargets', JSON.stringify(generatedTreeTargets));
  ...
  async function apply() {
    ...
    if (preview.kind === 'bundle' && !preserveBundleAreas && areaMode === 'existing' && !targetAreaId) return toast.error('请选择目标领域');
    if (preview.kind === 'bundle' && !preserveBundleAreas && areaMode === 'create' && !createAreaName.trim()) return toast.error('请输入新领域名称');
    ...
    try {
      const body = preview.kind === 'snapshot'
        ...
        : preview.kind === 'bundle' ? preserveBundleAreas
          ? { areaDecisions: (preview.suggestedAreaNames ?? []).map((sourceAreaName) => {
          ...
          : areaMode === 'existing' ? { targetAreaId } : { createAreaName }
        : preview.kind === 'tree-changeset' ? { ...(preview.revisionChanged && preview.treeFingerprint ? { confirmedTreeFingerprint: preview.treeFingerprint } : {}) }
        : { ...(preview.revisionChanged && preview.currentRevision ? { confirmedRevision: preview.currentRevision } : {}) };
      ...
      toast.success(`${preview.kind === 'snapshot' ? '新计划已导入' : preview.kind === 'bundle' ? '新计划树已导入' : '计划树增量已应用'}${autoActivated ? '，计划已根据节点状态自动设为进行中' : ''}`); await onApplied(plan); onClose();
    } catch (error) {
      if (error instanceof ApiClientError && error.code === 'TREE_REVISION_RECONFIRM_REQUIRED' && error.details) {
        const details = error.details as { currentTreeFingerprint: string };
        setPreview({ ...preview, revisionChanged: true, treeFingerprint: details.currentTreeFingerprint });
        toast.warning('计划树已变化，请检查预览后再次应用');
      } else if (error instanceof ApiClientError && error.code === 'REVISION_RECONFIRM_REQUIRED' && error.details) {
  ...
  return <Modal open={Boolean(mode)} onOpenChange={(open) => { if (!open) void close(); }}
    ...
    <div className="ai-workflow">
      <section className="ai-step"><div className="ai-step-title"><span>1</span><strong>描述需求</strong></div>
        ...
        {mode === 'snapshot' && <div className="segmented-control"><button type="button" className={newPlanKind === 'single' ? 'active' : ''} onClick={() => { setNewPlanKind('single'); setPrompt(''); setPreview(null); }}>单个计划</button><button type="button" className={newPlanKind === 'tree' ? 'active' : ''} onClick={() => { setNewPlanKind('tree'); setPrompt(''); setPreview(null); }}>完整计划树</button></div>}
        ...
        <details className="ai-advanced" open={advancedOpen} onToggle={(event) => setAdvancedOpen(event.currentTarget.open)}><summary>{mode === 'changeset' ? '上下文设置' : '高级选项'}</summary>
          ...
          : <><label>计划树范围<select value={treeScope} onChange={(event) => { setTreeScope(event.target.value as typeof treeScope); setPrompt(''); setPreview(null); setGeneratedTreeTargets([]); }}><option value="current">当前计划（全部节点）</option><option value="descendants">当前计划及全部后代</option><option value="custom">自定义当前计划节点</option></select></label>
            {treeScope !== 'descendants' && <div className="scope-control"><span>操作范围</span>
        ...
        <button className="primary-button" disabled={busy || contextBusy || (mode === 'changeset' && (!selectedPlan || (treeScope === 'custom' && !selectedKeys.length)))} onClick={makePrompt}><Sparkles size={17} />{contextBusy ? '正在构造' : '构造提示词'}</button>
      </section>
      ...
      {prompt && <section className="ai-step"><div className="ai-step-title"><span>2</span><strong>交给外部大模型</strong></div>
        ...
        <div className="inline-actions"><button className="secondary-button" onClick={() => copy(prompt, promptOutput.current)}><Clipboard size={16} />复制提示词</button><button className="secondary-button" onClick={() => downloadText(prompt, mode === 'snapshot' ? newPlanKind === 'tree' ? 'sixplan-new-plan-tree-prompt.txt' : 'sixplan-new-plan-prompt.txt' : 'sixplan-extend-plan-tree-prompt.txt')}><Download size={16} />下载 TXT</button></div>
      </section>}
      ...
      <section className="ai-step"><div className="ai-step-title"><span>{prompt ? '3' : '2'}</span><strong>校验模型返回</strong></div>
        ...
        <div className="inline-actions"><button className="secondary-button" disabled={busy || !raw.trim() || (mode === 'changeset' && !generatedTreeTargets.length)} onClick={() => validate(raw)}><FileJson size={16} />校验并预览</button><input ref={fileInput} type="file" accept=".json,.plan.json,.plan-bundle.json" hidden onChange={(event) => upload(event.target.files?.[0])} /><button className="secondary-button" disabled={busy || (mode === 'changeset' && (!planId || !generatedTreeTargets.length))} onClick={() => fileInput.current?.click()}><Upload size={16} />上传 JSON</button></div>
      ...
      {preview && <section className="ai-step preview-step"><div className="ai-step-title"><span><Check size={14} /></span><strong>确认预览</strong></div>
        ...
        {preview.kind === 'bundle' && <div className="area-decision"><label>领域方式<select value={preserveBundleAreas ? 'preserve' : 'single'} onChange={(event) => setPreserveBundleAreas(event.target.value === 'preserve')}><option value="preserve">保留模型给出的领域</option><option value="single">全部导入同一领域</option></select></label></div>}
        {(preview.kind === 'snapshot' || (preview.kind === 'bundle' && !preserveBundleAreas)) && <div className="area-decision"><label>目标方式<select value={areaMode} onChange={(event) => setAreaMode(event.target.value as 'existing' | 'create')}><option value="existing">选择已有领域</option><option value="create">创建新领域</option></select></label>{areaMode === 'existing' ? <label>目标领域<select value={targetAreaId} onChange={(event) => setTargetAreaId(event.target.value)}>{areas.map((area) => <option key={area.id} value={area.id}>{area.name}</option>)}</select></label> : <label>新领域名称<input value={createAreaName} onChange={(event) => setCreateAreaName(event.target.value)} /></label>}</div>}
        <button className={preview.removeNodeCount + preview.removeEdgeCount > 0 ? 'danger-button' : 'primary-button'} disabled={busy} onClick={apply}>{busy ? '正在应用' : preview.kind === 'snapshot' ? '创建新计划' : preview.kind === 'bundle' ? `创建计划树（${preview.planCount ?? 0} 个计划）` : '应用计划树变更'}</button>
      </section>}
    </div>
  </Modal>;
}
