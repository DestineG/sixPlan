...
任一节点可以创建或关联一个普通计划作为子计划。子计划仍会出现在活跃、全部和领域视图中，并显示父计划路径；一个节点最多关联一个子计划，一个计划最多有一个父节点，服务端会拒绝层级循环。

- 节点卡片显示子计划状态、完成节点数和归档状态，可一键进入；返回时恢复父计划节点选择与画布视口。
- 支持解除关联后保留或归档子计划、删除关联节点时保留或归档子计划，以及直接为整个子树更换父节点。
- 归档、恢复、跨领域移动和永久删除可选择仅当前计划或包含全部后代，并使用乐观并发检查。
- 单计划继续导出为 `.plan.json`；完整计划树导出为 `.plan-bundle.json`。导入计划树时可保留来源领域，或统一导入一个领域。
...
- `sixplan-plan-bundle` v1 创建包含节点关联的完整计划树；`sixplan-plan-tree-changeset` v1 增量更新已有计划树。
- 上传文件先流式写入临时目录，节点和连接逐项解析；临时会话默认 24 小时后清理。
- 所有写入在事务中再次检查日期、引用、重复边、环和图版本；删除操作使用红色预览并再次确认。
- 扩展计划默认使用当前计划全部节点，也可选择全部后代，或在只读 DAG 中自行勾选当前计划节点；可选携带目标节点现有 Markdown，并显示预计内容大小。
